...
function printTable() {
    ...
    // Prints the content of the table. First the week numbers, then the chofers.
    for (var i = 1; i <= NUM_WEEKS; i++) {
        table += '<tr>';
        table += printWeek(i);
        // console.log("semana " + i);

        for (var j = 0; j < camiones.length; j++) {
            let choferIndex = (i - 1 + j) % chofers.length;
            table += printChofer(choferIndex);
...
function printCamion(camionIndex) {
    return '<th>' + camiones[camionIndex] + '</th>';
}
...
function printChofer(choferIndex) {
    return '<td>' + chofers[choferIndex] + '</td>';
}
